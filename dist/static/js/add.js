global.webpackJsonp([1],{

/***/ 0:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "e", function() { return toast; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return showLoading; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "d", function() { return hideLoading; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return modal; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return getStorage; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "g", function() { return setStorage; });
/* unused harmony export chooseImg */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "h", function() { return wxLogin; });
/* unused harmony export upLoad */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "f", function() { return jumpTo; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise__ = __webpack_require__(4);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise__);

var FILEURL = ''; //  文件服务器地址

var toast = function toast() {
  var title = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '提示';
  var icon = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'success';
  var duration = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1500;
  var mask = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;

  return new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
    wx.showToast({
      title: title,
      icon: icon,
      duration: duration,
      mask: mask,
      success: function success(res) {
        return resolve(res);
      },
      fail: function fail(err) {
        return reject(err);
      }
    });
  });
};

var showLoading = function showLoading() {
  var title = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '加载中';
  var mask = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

  return new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
    wx.showLoading({
      title: title,
      success: function success(res) {
        return resolve(res);
      },
      fail: function fail(err) {
        return reject(err);
      }
    });
  });
};

var hideLoading = function hideLoading() {
  wx.hideLoading();
};

var modal = function modal() {
  var title = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '提示';
  var content = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';

  return new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
    wx.showModal({
      title: title,
      content: content,
      success: function success(res) {
        if (res.confirm) {
          resolve(res);
        } else {
          reject();
        }
      },
      fail: function fail(err) {
        return reject(err);
      }
    });
  });
};

var getStorage = function getStorage(item) {
  return wx.getStorageSync(item);
};

var setStorage = function setStorage(key, value) {
  wx.setStorageSync(key, value);
};

var chooseImg = function chooseImg() {
  var count = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 9;

  return new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
    wx.chooseImage({
      count: count,
      success: function success(res) {
        return resolve(res);
      },
      fail: function fail(err) {
        return reject(err);
      }
    });
  });
};
var wxLogin = function wxLogin() {
  return new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
    wx.login({
      success: function success(res) {
        return resolve(res);
      },
      fail: function fail(err) {
        return reject(err);
      }
    });
  });
};

var upLoad = function upLoad(filePath, formData) {
  return new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
    wx.uploadFile({
      url: FILEURL,
      filePath: filePath, //  本地路径名
      name: 'file',
      formData: formData,
      success: function success(res) {
        return resolve(res);
      },
      fail: function fail(err) {
        return reject(err);
      }
    });
  });
};
var jumpTo = function jumpTo(url) {
  var state = url.indexOf('personalCenter') !== -1 || url.indexOf('add') !== -1 || url.indexOf('find') !== -1;
  if (state) {
    wx.switchTab({
      url: url
    });
  } else {
    wx.navigateTo({
      url: url
    });
  }
};


/***/ }),

/***/ 100:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["a"] = ({
  data: function data() {
    return {
      data: {}
    };
  },

  props: {
    menuList: {
      type: Array,
      default: []
    },
    type: {
      type: String,
      default: 'daily'
    }
  },
  methods: {
    tabClick: function tabClick(e) {
      this.$emit('changeTaskType', e.target.id);
    }
  }
});

/***/ }),

/***/ 104:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
//
//
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["a"] = ({
  props: {
    info: {
      type: Object
    },
    index: {
      type: Number
    }
  },
  methods: {
    changeTextarea: function changeTextarea(e) {
      this.$emit('changeTaskItemContent', e.target.id, e.target.value);
    },
    deleteTexterea: function deleteTexterea(e) {
      this.$emit('deleteTaskItem', parseInt(e.target.id));
    }
  }
});

/***/ }),

/***/ 105:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_tab__ = __webpack_require__(194);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__components_modal__ = __webpack_require__(189);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__components_input__ = __webpack_require__(188);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__components_textarea__ = __webpack_require__(198);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__components_button__ = __webpack_require__(184);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__components_formButton__ = __webpack_require__(42);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__utils_utils__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__api_API__ = __webpack_require__(5);
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//











/* harmony default export */ __webpack_exports__["a"] = ({
  data: function data() {
    return {
      info: {
        type: 'daily',
        taskName: {
          name: '任务名称',
          type: 'input',
          value: ''
        },
        startDate: {
          id: 'startDate',
          name: '任务开始日期',
          type: 'date',
          value: '',
          start: '',
          end: ''
        },
        endDate: {
          id: 'endDate',
          name: '任务结束日期',
          type: 'date',
          value: '',
          start: '',
          end: ''
        },
        startTime: {
          id: 'startTime',
          name: '每日开始时间',
          type: 'time',
          value: '',
          start: '',
          end: ''
        },
        endTime: {
          id: 'endTime',
          name: '每日结束时间',
          type: 'time',
          value: '',
          start: '',
          end: ''
        },
        players: {
          id: 'players',
          name: '最大参与人数',
          type: 'picker',
          value: 4,
          data: [2, 4, 6, 8, 10, 12, 20, 50, 99]
        },
        taskList: [],
        public: {
          id: 'isPublic',
          name: '是否公开',
          type: 'picker',
          value: 0,
          data: ['否', '是']
        }
      },
      taskInfo: {},
      hidden: true,
      groupId: 66366,
      itemId: 1,
      formId: '',
      buttonContent: '新建任务',
      canClick: true
    };
  },

  components: {
    tab: __WEBPACK_IMPORTED_MODULE_0__components_tab__["a" /* default */],
    tcInput: __WEBPACK_IMPORTED_MODULE_2__components_input__["a" /* default */],
    tcTextarea: __WEBPACK_IMPORTED_MODULE_3__components_textarea__["a" /* default */],
    tcButton: __WEBPACK_IMPORTED_MODULE_4__components_button__["a" /* default */],
    Modal: __WEBPACK_IMPORTED_MODULE_1__components_modal__["a" /* default */],
    formButton: __WEBPACK_IMPORTED_MODULE_5__components_formButton__["a" /* default */]
  },
  methods: {
    changeTaskType: function changeTaskType(type) {
      this.info.type = type;
    },
    addSubTask: function addSubTask(e) {
      this.info.taskList.push({
        id: this.itemId++,
        value: ''
      });
    },
    checkTaskName: function checkTaskName() {
      return this.info.taskName.value.length > 0;
    },
    checkTaskInfo: function checkTaskInfo() {
      if (this.info.taskList.length === 0) {
        return false;
      }
      return this.info.taskList.every(function (item) {
        return item.value.length > 0;
      });
    },
    getTaskInfo: function getTaskInfo() {
      try {
        this.taskInfo = {
          title: this.info.taskName.value,
          startTime: __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["d" /* cst2unix */])(this.info.startDate.value + 'T' + this.info.startTime.value + ':00'),
          endTime: __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["d" /* cst2unix */])(this.info.endDate.value + 'T' + this.info.endTime.value + ':00'),
          isPublic: this.info.public.value === '1',
          maxPeople: 1,
          items: this.info.taskList.map(function (item) {
            return {
              content: item.value
            };
          }),
          type: 1
        };
        if (this.info.type === 'multiplayer') {
          this.taskInfo.maxPeople = this.info.players.data[this.info.players.value];
          this.taskInfo.type = 0;
        }
      } catch (e) {
        throw new Error('有空值');
      }
      return this;
    },
    clearTask: function clearTask() {
      var data = this.info;
      this.taskInfo = {};
      data.public.value = 0;
      data.taskName.value = '';
      data.taskList = [];
      this.initDate();
      this.initTime();
    },
    submitTask: function submitTask() {
      var _this = this;

      __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["c" /* showLoading */])('提交中');
      this.canClick = false;
      __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_8__api_API__["q" /* CreateNewTask */])(this.taskInfo).then(function (res) {
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["d" /* hideLoading */])();
        _this.canClick = true;
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_8__api_API__["f" /* SubmitForm */])({ groupId: res.data.groupId, formId: _this.formId, type: 0 }).then(function (res) {
          console.log(res);
        }).catch(function () {
          __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["e" /* toast */])('消息通知失败', 'none');
        });
        if (_this.taskInfo.type === 0) {
          _this.hidden = false;
          _this.groupId = res.data.groupId;
        } else {
          __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["e" /* toast */])('创建成功');
          setTimeout(function () {
            _this.clearTask();
            __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["f" /* jumpTo */])('../personalCenter/personalCenter');
          }, 1000);
        }
      }).catch(function (err) {
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["e" /* toast */])('提交任务失败', 'none');
        _this.canClick = false;
        console.error(err);
      });
    },
    createTask: function createTask() {
      var _this2 = this;

      if (this.checkTaskName() && this.checkTaskInfo()) {
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["b" /* modal */])('创建新任务', '确认无误后，即可提交。').then(function (res) {
          _this2.getTaskInfo().submitTask();
        });
      } else {
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["e" /* toast */])('请填写相关信息', 'none');
      }
    },
    shareTask: function shareTask(key) {
      if (key !== 'share') {
        this.clearTask();
        this.hidden = true;
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["f" /* jumpTo */])('../personalCenter/personalCenter');
      }
    },
    getFormId: function getFormId(key) {
      this.formId = key;
      this.createTask();
    },
    initDate: function initDate() {
      this.info.startDate.value = '请设置';
      this.info.startDate.start = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["e" /* unix2cst */])(Date.now());
      this.info.startDate.end = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["e" /* unix2cst */])(Date.now() + 3600 * 1000 * 24 * 90);

      this.info.endDate.start = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["e" /* unix2cst */])(Date.now());
      this.info.endDate.value = '请设置';
      this.info.endDate.end = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["e" /* unix2cst */])(Date.now() + 3600 * 1000 * 24 * 90);
    },
    changeStartDate: function changeStartDate(value) {
      this.info.startDate.value = value;
      this.info.endDate.value = value;
      this.info.endDate.start = value;
      this.info.endDate.end = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["e" /* unix2cst */])(__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__utils_utils__["d" /* cst2unix */])(value) + 3600 * 1000 * 24 * 90);
    },
    changeEndDate: function changeEndDate(value) {
      this.info.endDate.value = value;
    },
    initTime: function initTime() {
      this.info.startTime.value = '请设置';
      this.info.endTime.value = '请设置';
    },
    changeStartTime: function changeStartTime(value) {
      this.info.startTime.value = value;
      this.info.endTime.start = value;
      if (this.info.startTime.value !== '请设置' && this.info.endTime.value < this.info.startTime.value) {
        this.info.endTime.value = value;
      }
    },
    changeEndTime: function changeEndTime(value) {
      this.info.endTime.value = value;
      this.info.startTime.end = value;
    },
    changeTaskName: function changeTaskName(value) {
      this.info.taskName.value = value;
    },
    deleteTaskItem: function deleteTaskItem(key) {
      this.info.taskList = this.info.taskList.filter(function (item) {
        return item.id !== key;
      });
    },
    changeTaskItemContent: function changeTaskItemContent(key, value) {
      var index = this.findTask(key);
      if (index !== -1) {
        this.info.taskList[index].value = value;
      }
    },
    findTask: function findTask(key) {
      var itemIndex = -1;
      this.info.taskList.forEach(function (item, index) {
        if (item.id === parseInt(key)) {
          itemIndex = index;
        }
      });
      return itemIndex;
    },
    changeIsPublic: function changeIsPublic(value) {
      this.info.public.value = value;
    },
    changePlayerNum: function changePlayerNum(value) {
      this.info.players.value = value;
    }
  },
  onShareAppMessage: function onShareAppMessage(options) {
    return {
      title: '一起来挑战吧',
      path: '/pages/simpleTask/simpleTask?groupId=' + this.groupId + '&share=true'
    };
  },
  beforeMount: function beforeMount() {
    this.initDate();
    this.initTime();
  },
  onShow: function onShow() {
    __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["c" /* showLoading */])();
    setTimeout(function () {
      __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_6__utils_wxUtils__["d" /* hideLoading */])();
    }, 1000);
  }
});

/***/ }),

/***/ 165:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 172:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 174:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 175:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 176:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 180:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 184:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_button_vue__ = __webpack_require__(90);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_df46d0e2_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_button_vue__ = __webpack_require__(223);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(180)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_button_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_df46d0e2_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_button_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/components/button.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] button.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-df46d0e2", Component.options)
  } else {
    hotAPI.reload("data-v-df46d0e2", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 188:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_input_vue__ = __webpack_require__(94);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_7707fc06_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_input_vue__ = __webpack_require__(215);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(172)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_input_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_7707fc06_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_input_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/components/input.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] input.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-7707fc06", Component.options)
  } else {
    hotAPI.reload("data-v-7707fc06", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 189:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_modal_vue__ = __webpack_require__(95);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_3fdb0940_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_modal_vue__ = __webpack_require__(208);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(165)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_modal_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_3fdb0940_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_modal_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/components/modal.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] modal.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-3fdb0940", Component.options)
  } else {
    hotAPI.reload("data-v-3fdb0940", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 194:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_tab_vue__ = __webpack_require__(100);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_92c811f0_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_tab_vue__ = __webpack_require__(218);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(175)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_tab_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_92c811f0_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_tab_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/components/tab.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] tab.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-92c811f0", Component.options)
  } else {
    hotAPI.reload("data-v-92c811f0", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 198:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_textarea_vue__ = __webpack_require__(104);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_7ac84657_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_textarea_vue__ = __webpack_require__(217);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(174)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_textarea_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_7ac84657_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_textarea_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/components/textarea.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] textarea.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-7ac84657", Component.options)
  } else {
    hotAPI.reload("data-v-7ac84657", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 2:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* unused harmony export throttle */
/* unused harmony export debounce */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return normalizeTime; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "e", function() { return unix2cst; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "d", function() { return cst2unix; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return normalizeTimeHours; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return parseToken; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_jsrsasign__ = __webpack_require__(13);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_jsrsasign___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_jsrsasign__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__wxUtils__ = __webpack_require__(0);



/*  函数节流  */
function throttle(fn, delay) {
  var prev = Date.now();
  return function () {
    var now = Date.now();
    var args = arguments;
    if (now - prev >= delay) {
      fn.apply(this, args);
      prev = Date.now();
    }
  };
}

/*  函数防抖  */
function debounce(fn, delay) {
  var timer = null;
  return function () {
    var context = this;
    var args = arguments;
    clearTimeout(timer);
    timer = setTimeout(function () {
      fn.apply(context, args);
    }, delay);
  };
}
function unix2cst(time) {
  var date = void 0;
  date = new Date(time).toJSON().replace(/T/, ' ').split('.')[0].slice(0, -3);
  return date;
}
function cst2unix(time) {
  return new Date(time).getTime();
}
function normalizeTime(time) {
  return time.split('T')[0];
}
function normalizeTimeHours(time) {
  return time.split('.')[0].replace(/T/, ' ').slice(0, -3);
}
var parseToken = function parseToken(token) {
  // 小程序不支持atob，所有引入了jsrsasign库的 b64utoutf8
  var info = JSON.parse(__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_jsrsasign__["b64utoutf8"])(token.split('.')[1]));
  __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__wxUtils__["g" /* setStorage */])('exp', info.exp * 1000);
  __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__wxUtils__["g" /* setStorage */])('userId', info.userId);
};


/***/ }),

/***/ 208:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "tc-modal",
    attrs: {
      "hidden": _vm.hidden === undefined || _vm.hidden,
      "eventid": '0'
    },
    on: {
      "click": _vm.modalClick
    }
  }, [_c('div', {
    staticClass: "tc-modal-body"
  }, [_c('div', {
    staticClass: "tc-modal-body-content"
  }, [_vm._v("创建成功！")]), _vm._v(" "), _c('div', {
    staticClass: "tc-modal-body-options"
  }, [_c('button', {
    staticClass: "tc-modal-body-options-item tc-modal-body-options-item-font",
    attrs: {
      "open-type": "share",
      "id": "share"
    }
  }, [_vm._v("分享到微信")]), _vm._v(" "), _c('button', {
    staticClass: "tc-modal-body-options-item tc-modal-body-options-item-font"
  }, [_vm._v("关闭")])], 1)])])
}
var staticRenderFns = []
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-3fdb0940", esExports)
  }
}

/***/ }),

/***/ 215:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "tc-input"
  }, [_c('label', [_vm._v(_vm._s(_vm.info.name))]), _vm._v(" "), (_vm.info.type === 'input') ? _c('input', {
    attrs: {
      "value": _vm.info.value,
      "placeholder": "请输入",
      "eventid": '0',
      "eventid": '1'
    },
    on: {
      "input": _vm.changeInput
    }
  }) : _vm._e(), _vm._v(" "), (_vm.info.type === 'picker') ? _c('picker', {
    attrs: {
      "value": _vm.info.value,
      "range": _vm.info.data,
      "id": _vm.info.id,
      "eventid": '2',
      "eventid": '3'
    },
    on: {
      "change": _vm.changeValue
    }
  }, [_vm._v("\n   " + _vm._s(_vm.info.data[_vm.info.value]) + "\n  ")]) : _vm._e(), _vm._v(" "), (_vm.info.type === 'date') ? _c('picker', {
    attrs: {
      "value": _vm.info.value,
      "placeholder": "请设置",
      "mode": "date",
      "start": _vm.info.start,
      "end": _vm.info.end,
      "id": _vm.info.id,
      "eventid": '4',
      "eventid": '5'
    },
    on: {
      "change": _vm.changeDate
    }
  }, [_vm._v("\n    " + _vm._s(_vm.info.value) + "\n  ")]) : _vm._e(), _vm._v(" "), (_vm.info.type === 'time') ? _c('picker', {
    attrs: {
      "value": _vm.info.value,
      "placeholder": "请设置",
      "mode": "time",
      "start": _vm.info.start,
      "end": _vm.info.end,
      "id": _vm.info.id,
      "eventid": '6',
      "eventid": '7'
    },
    on: {
      "change": _vm.changeTime
    }
  }, [_vm._v("\n    " + _vm._s(_vm.info.value) + "\n  ")]) : _vm._e()], 1)
}
var staticRenderFns = []
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-7707fc06", esExports)
  }
}

/***/ }),

/***/ 217:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "tc-textarea"
  }, [_c('label', [_vm._v("任务" + _vm._s(_vm.index + 1))]), _vm._v(" "), _c('textarea', {
    attrs: {
      "value": _vm.info.value,
      "placeholder": "30字以内",
      "maxlength": "30",
      "id": _vm.info.id,
      "eventid": '0'
    },
    on: {
      "input": _vm.changeTextarea
    }
  }), _vm._v(" "), _c('img', {
    attrs: {
      "src": "/static/images/delete.png",
      "id": _vm.info.id,
      "eventid": '1'
    },
    on: {
      "click": _vm.deleteTexterea
    }
  })], 1)
}
var staticRenderFns = []
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-7ac84657", esExports)
  }
}

/***/ }),

/***/ 218:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "tc-tab",
    attrs: {
      "eventid": '0'
    },
    on: {
      "click": _vm.tabClick
    }
  }, [_c('div', {
    staticClass: "tc-tab-item",
    class: [
      _vm.type === 'daily' ? 'tc-tab-item-active' : ''
    ],
    attrs: {
      "id": "daily"
    }
  }, [_vm._v("单人")]), _vm._v(" "), _c('div', {
    staticClass: "tc-tab-item",
    class: [
      _vm.type === 'multiplayer' ? 'tc-tab-item-active' : ''
    ],
    attrs: {
      "id": "multiplayer"
    }
  }, [_vm._v("多人")])])
}
var staticRenderFns = []
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-92c811f0", esExports)
  }
}

/***/ }),

/***/ 219:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('div', {
    staticClass: "add"
  }, [_c('tab', {
    attrs: {
      "type": _vm.info.type,
      "eventid": '0',
      "mpcomid": '0'
    },
    on: {
      "changeTaskType": _vm.changeTaskType
    }
  }), _vm._v(" "), _c('div', [_c('tcInput', {
    attrs: {
      "info": _vm.info.taskName,
      "eventid": '1',
      "mpcomid": '1'
    },
    on: {
      "changeTaskName": _vm.changeTaskName
    }
  }), _vm._v(" "), _vm._l((_vm.info.taskList), function(item, index) {
    return _c('tcTextarea', {
      key: item.id,
      attrs: {
        "info": item,
        "index": index,
        "eventid": '2-' + index,
        "mpcomid": '2-' + index
      },
      on: {
        "changeTaskItemContent": _vm.changeTaskItemContent,
        "deleteTaskItem": _vm.deleteTaskItem
      }
    })
  }), _vm._v(" "), _c('div', {
    staticClass: "add-addTaskItem"
  }, [_c('img', {
    attrs: {
      "src": "/static/images/addButton.png",
      "id": "img",
      "eventid": '3'
    },
    on: {
      "click": _vm.addSubTask
    }
  })]), _vm._v(" "), _vm._m(0), _vm._v(" "), _c('div', {
    staticClass: "add-info-margin"
  }), _vm._v(" "), _c('tcInput', {
    attrs: {
      "info": _vm.info.startDate,
      "eventid": '4',
      "mpcomid": '3'
    },
    on: {
      "changeStartDate": _vm.changeStartDate
    }
  }), _vm._v(" "), _c('tcInput', {
    attrs: {
      "info": _vm.info.endDate,
      "eventid": '5',
      "mpcomid": '4'
    },
    on: {
      "changeEndDate": _vm.changeEndDate
    }
  }), _vm._v(" "), _c('tcInput', {
    attrs: {
      "info": _vm.info.startTime,
      "eventid": '6',
      "mpcomid": '5'
    },
    on: {
      "changeStartTime": _vm.changeStartTime
    }
  }), _vm._v(" "), _c('tcInput', {
    attrs: {
      "info": _vm.info.endTime,
      "eventid": '7',
      "mpcomid": '6'
    },
    on: {
      "changeEndTime": _vm.changeEndTime
    }
  })], 2), _vm._v(" "), (_vm.info.type === 'multiplayer') ? _c('div', [_c('tcInput', {
    attrs: {
      "info": _vm.info.players,
      "eventid": '8',
      "mpcomid": '7',
      "eventid": '10',
      "mpcomid": '9'
    },
    on: {
      "changePlayerNum": _vm.changePlayerNum
    }
  }), _vm._v(" "), _c('tcInput', {
    attrs: {
      "info": _vm.info.public,
      "eventid": '9',
      "mpcomid": '8',
      "eventid": '11',
      "mpcomid": '10'
    },
    on: {
      "changeIsPublic": _vm.changeIsPublic
    }
  })], 1) : _vm._e()], 1), _vm._v(" "), _c('div', {
    staticStyle: {
      "padding-top": "15rpx",
      "padding-bottom": "15rpx"
    }
  }, [_c('formButton', {
    attrs: {
      "buttonContent": _vm.buttonContent,
      "active": _vm.canClick,
      "eventid": '12',
      "mpcomid": '11'
    },
    on: {
      "getFormId": _vm.getFormId
    }
  })], 1), _vm._v(" "), _c('Modal', {
    attrs: {
      "hidden": _vm.hidden,
      "eventid": '13',
      "mpcomid": '12'
    },
    on: {
      "shareTask": _vm.shareTask
    }
  })], 1)
}
var staticRenderFns = [function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "add-addTaskItem-info"
  }, [_vm._v("\n        添加任务项，获得多倍积分, 完成后次日到账"), _c('img', {
    staticClass: "add-addTaskItem-info-tag",
    attrs: {
      "src": "/static/images/jifen.png"
    }
  })])
}]
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-bb4716c6", esExports)
  }
}

/***/ }),

/***/ 223:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "tc-button",
    attrs: {
      "id": "button",
      "eventid": '0'
    },
    on: {
      "click": _vm.handleClick
    }
  }, [_c('img', {
    attrs: {
      "src": "/static/images/button.png"
    }
  }), _vm._v(" "), _c('div', {
    staticClass: "tc-button-info"
  }, [_vm._t("default", null, {
    mpcomid: '0'
  })], 2)])
}
var staticRenderFns = []
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-df46d0e2", esExports)
  }
}

/***/ }),

/***/ 28:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["a"] = ({
  props: {
    buttonContent: {
      type: String,
      default: '创建任务'
    },
    active: {
      type: Boolean,
      default: false
    }
  },
  methods: {
    submit: function submit(e) {
      this.$emit('getFormId', e.target.formId);
    }
  }
});

/***/ }),

/***/ 39:
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ 42:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_formButton_vue__ = __webpack_require__(28);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_39ed70f3_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_formButton_vue__ = __webpack_require__(45);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(39)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_formButton_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_39ed70f3_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_formButton_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/components/formButton.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] formButton.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-39ed70f3", Component.options)
  } else {
    hotAPI.reload("data-v-39ed70f3", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 45:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('form', {
    staticClass: "tc-formButton",
    class: _vm.type ? 'return' : '',
    attrs: {
      "report-submit": "true",
      "eventid": '0'
    },
    on: {
      "submit": _vm.submit
    }
  }, [_c('button', {
    attrs: {
      "formType": "submit",
      "disabled": !_vm.active
    }
  }, [_vm._v(_vm._s(_vm.buttonContent || '创建任务'))])], 1)
}
var staticRenderFns = []
render._withStripped = true
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ __webpack_exports__["a"] = (esExports);
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-39ed70f3", esExports)
  }
}

/***/ }),

/***/ 5:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* unused harmony export DownLoad */
/* unused harmony export UpLoad */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "p", function() { return UpdateUserInfo; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "k", function() { return Login; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "j", function() { return GetScore; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "l", function() { return GetHistory; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "i", function() { return GetUserInfo; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "h", function() { return GetCurrentTask; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "q", function() { return CreateNewTask; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "n", function() { return GetRank; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return GetTaskInfo; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return FinishTaskItem; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "m", function() { return GetFutureTaskList; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "d", function() { return GetSimpleTaskInfo; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "e", function() { return JoinTaskGroup; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return GetOthersTaskInfo; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "o", function() { return GetTaskVillage; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "g", function() { return GetScoreHistory; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "f", function() { return SubmitForm; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_json_stringify__ = __webpack_require__(12);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_json_stringify___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_json_stringify__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__ajax__ = __webpack_require__(6);


var Login = function Login(code) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/user/login', 'post', __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_json_stringify___default()({ code: code }));
};
var GetUserInfo = function GetUserInfo() {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/user/info');
};
var GetScore = function GetScore() {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/user/score', 'get');
};
var GetCurrentTask = function GetCurrentTask() {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/pending', 'get');
};
var UpLoad = function UpLoad(data) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('', '', data);
};
var DownLoad = function DownLoad(data) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('', 'get', data);
};
var UpdateUserInfo = function UpdateUserInfo(data) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/user/info', 'put', data);
};
var CreateNewTask = function CreateNewTask(data) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/group', 'post', data);
};
var GetHistory = function GetHistory(id) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/history', 'get_restful', { id: id });
};
var GetRank = function GetRank(page) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/user/rank', 'get_restful', { id: page });
};
var GetTaskInfo = function GetTaskInfo(id) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/group', 'get_restful', { id: id });
};
var FinishTaskItem = function FinishTaskItem(data) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/item', 'post', data);
};
var GetFutureTaskList = function GetFutureTaskList() {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/not-start', 'get');
};
var GetSimpleTaskInfo = function GetSimpleTaskInfo(id) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/detail/simple', 'get_restful', { id: id });
};
var JoinTaskGroup = function JoinTaskGroup(id) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/group/join', 'post', { groupId: id });
};
var GetOthersTaskInfo = function GetOthersTaskInfo(ids) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/group', 'get_restful2', ids);
};
var GetTaskVillage = function GetTaskVillage() {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/task/village', 'get');
};
var GetScoreHistory = function GetScoreHistory(ids) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/user/score/history', 'get_restful2', ids);
};
var SubmitForm = function SubmitForm(data) {
  return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__ajax__["a" /* default */])('/formId', 'post', data);
};


/***/ }),

/***/ 6:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_helpers_extends__ = __webpack_require__(9);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_runtime_helpers_extends___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_babel_runtime_helpers_extends__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_promise__ = __webpack_require__(4);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_promise___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_promise__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__mock_route__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__utils_wxUtils__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__utils_utils__ = __webpack_require__(2);






var BASEURL = 'https://hotpot.zhengsj.top';
var DEBUG = false;
var methodsMap = {
  GET: function GET(url, methods, data) {
    return {
      url: '' + BASEURL + url,
      method: methods
    };
  },
  GET_RESTFUL: function GET_RESTFUL(url, methods, data) {
    return {
      url: '' + BASEURL + url + '/' + data.id,
      method: 'GET'
    };
  },
  POST: function POST(url, methods, data) {
    return {
      url: '' + BASEURL + url,
      method: methods,
      data: data
    };
  },
  PUT: function PUT(url, methods, data) {
    return {
      url: '' + BASEURL + url,
      method: methods,
      data: data
    };
  },
  GET_RESTFUL2: function GET_RESTFUL2(url, methods, data) {
    var str = '';
    data.forEach(function (item) {
      str += '/' + item;
    });
    return {
      url: '' + BASEURL + url + str,
      methods: 'GET'
    };
  }
};
var configRequest = function configRequest(url, methods, data) {
  return methodsMap[methods.toUpperCase()](url, methods.toUpperCase(), data);
};
var ajax = function ajax(url) {
  var methods = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'GET';
  var data = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var headers = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : { 'Content-Type': 'application/json' };

  if (!DEBUG) {
    return new __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
      var obj = configRequest(url, methods, data);
      if (__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_3__utils_wxUtils__["a" /* getStorage */])('token')) {
        headers['Authorization'] = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_3__utils_wxUtils__["a" /* getStorage */])('token');
      }
      wx.request(__WEBPACK_IMPORTED_MODULE_0_babel_runtime_helpers_extends___default()({}, obj, {
        header: headers,
        success: function success(res) {
          if (typeof res.data !== 'undefined' && typeof res.data.data !== 'undefined' && res.data.data.token) {
            __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_3__utils_wxUtils__["g" /* setStorage */])('token', res.data.data.token);
            __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_4__utils_utils__["b" /* parseToken */])(res.data.data.token);
          }
          if (typeof res.data !== 'undefined' && res.data.status === 0) {
            resolve(res.data);
          } else {
            reject(res.data);
          }
        },
        fail: function fail(err) {
          return reject(err);
        }
      }));
    });
  } else {
    var res = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_2__mock_route__["a" /* default */])(url);
    return new __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_promise___default.a(function (resolve, reject) {
      resolve(res);
    });
  }
};
/* harmony default export */ __webpack_exports__["a"] = (ajax);

/***/ }),

/***/ 65:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_index_vue__ = __webpack_require__(105);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_bb4716c6_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_index_vue__ = __webpack_require__(219);
var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__(176)
}
var normalizeComponent = __webpack_require__(1)
/* script */

/* template */

/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_mpvue_loader_lib_selector_type_script_index_0_index_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_mpvue_loader_lib_template_compiler_index_id_data_v_bb4716c6_hasScoped_false_transformToRequire_video_src_source_src_img_src_image_xlink_href_node_modules_mpvue_loader_lib_selector_type_template_index_0_index_vue__["a" /* default */],
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/pages/add/index.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
if (Component.options.functional) {console.error("[vue-loader] index.vue: functional components are not supported with templates, they should use render functions.")}

/* hot reload */
if (false) {(function () {
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), false)
  if (!hotAPI.compatible) return
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-bb4716c6", Component.options)
  } else {
    hotAPI.reload("data-v-bb4716c6", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
})()}

/* harmony default export */ __webpack_exports__["a"] = (Component.exports);


/***/ }),

/***/ 7:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (immutable) */ __webpack_exports__["a"] = Route;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__templates__ = __webpack_require__(8);


function Route(url) {
  var method = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'GET';

  return __WEBPACK_IMPORTED_MODULE_0__templates__["a" /* default */][url];
}

/***/ }),

/***/ 77:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_vue__ = __webpack_require__(3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__index__ = __webpack_require__(65);



var app = new __WEBPACK_IMPORTED_MODULE_0_vue___default.a(__WEBPACK_IMPORTED_MODULE_1__index__["a" /* default */]);
app.$mount();
/* harmony default export */ __webpack_exports__["default"] = ({
  config: {
    navigationBarTitleText: '新建任务'
  }
});

/***/ }),

/***/ 8:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_mockjs__ = __webpack_require__(14);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_mockjs___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_mockjs__);


var userInfo = __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.mock({
  'nickName': __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.first(),
  'avatar': 'http://5b0988e595225.cdn.sohucs.com/images/20170911/aa2247ad3e9345809e4bc1cb3c3f30fe.jpeg',
  'contend|1-1000': 1,
  'daily|1-10000': 1
});
var taskList = __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.mock({
  'data|0-3': [{
    'taskName|+1': ['软件工程大作业', '数据库实验3', 'OOP实验2'],
    'id|1-100': 0,
    'type|1': ['daily', 'multiPlayer'],
    'taskNum|5-8': 0,
    'finishedTaskNum|1-6': 0,
    'finishedPlayerNum|0-3': 0,
    'deadLine|+1': [__WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd H:mm'), __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd H:mm'), __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd H:mm')]
  }]
});

var moreUserInfo = __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.mock({
  'data': {
    'avatar|1': ['http://5b0988e595225.cdn.sohucs.com/images/20170911/aa2247ad3e9345809e4bc1cb3c3f30fe.jpeg', 'data:image/jpeg;base64,', 'data:image/jpeg;base64,', 'data:image/jpeg;base64,', 'data:image/jpeg;base64,', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQB9SUBVkbeP-DLK3fTjGPx_9BGl_lk-zrFR6WUhCBwJodNAXGr'],
    'gender|1': ['man', 'woman'],
    'date|1': __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd'),
    'location|0-1': 0,
    'university|1': ['清华大学', '北京大学', '西安电子科技大学', '华中科技大学', '西安外国语大学'],
    'degree|0-5': 0
  }
});

var historyTask = __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.mock({
  'data|3-10': [{
    'taskName|+1': ['软件工程大作业', '数据库实验3', 'OOP实验2'],
    'id|1-100': 0,
    'type|1': ['daily', 'multiPlayer'],
    'finishedPlayerNum|0-3': 0,
    'deadLine|+1': [__WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd H:mm'), __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd H:mm'), __WEBPACK_IMPORTED_MODULE_0_mockjs___default.a.Random.date('yyyy-MM-dd H:mm')],
    'hasFinished|1': [true, false]
  }]
});
var templatesMap = {
  '/userInfo': userInfo,
  '/taskList': taskList,
  '/moreUserInfo': moreUserInfo,
  '/historyTask': historyTask
};

/* harmony default export */ __webpack_exports__["a"] = (templatesMap);

/***/ }),

/***/ 90:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
//
//
//
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["a"] = ({
  methods: {
    handleClick: function handleClick(e) {
      console.log('我点击的是 Button 组件， 触发父组件buttonClick事件');
      this.$emit('buttonClick', e);
    }
  }
});

/***/ }),

/***/ 94:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["a"] = ({
  props: {
    info: {
      type: Object
    }
  },
  methods: {
    changeDate: function changeDate(e) {
      if (e.target.id === 'startDate') {
        this.$emit('changeStartDate', e.target.value);
      } else if (e.target.id === 'endDate') {
        this.$emit('changeEndDate', e.target.value);
      }
    },
    changeTime: function changeTime(e) {
      if (e.target.id === 'startTime') {
        this.$emit('changeStartTime', e.target.value);
      } else if (e.target.id === 'endTime') {
        this.$emit('changeEndTime', e.target.value);
      }
    },
    changeValue: function changeValue(e) {
      console.log(e);
      if (e.target.id === 'players') {
        this.$emit('changePlayerNum', e.target.value);
      } else if (e.target.id === 'isPublic') {
        this.$emit('changeIsPublic', e.target.value);
      }
    },
    changeInput: function changeInput(e) {
      this.$emit('changeTaskName', e.target.value);
    }
  },
  create: function create() {}
});

/***/ }),

/***/ 95:
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
//
//
//
//
//
//
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["a"] = ({
  props: {
    content: {
      type: String,
      default: '创建成功'
    },
    hidden: {
      type: Boolean,
      default: true
    }
  },
  methods: {
    modalClick: function modalClick(e) {
      if (e.target.id === '') {
        this.hidden = true;
        this.$emit('shareTask', 'close');
      } else {
        this.$emit('shareTask', e.target.id);
      }
    }
  }
});

/***/ })

},[77]);
//# sourceMappingURL=add.js.map